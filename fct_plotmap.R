...
"image.cont.ano" <- function(lonF, latF, champ, titre = "", Ichange = numeric(0), zlev = seq(min(champ, na.rm = TRUE), max(champ,na.rm = TRUE), length = 11),
                             transpose = TRUE, mar = c(5, 5, 5, 6), legend = TRUE, xlab = "Longitude", ylab = "Latitude",
                             satur = FALSE, paquet = "fields", add = FALSE, lonrange = range(lonF), latrange = range(latF)) {
  zmax <- max(abs(min(champ,na.rm=TRUE)),abs(max(champ,na.rm=TRUE)))
  zlev <- seq(-zmax,zmax,length=12)
  col11 <- c('#67001f','#b2182b','#d6604d','#f4a582','#fddbc7','#f7f7f7','#d1e5f0','#92c5de','#4393c3','#2166ac','#053061')
  # par( mar=c(10,5,5,5))
  par(mar = mar)
  if (satur) {
    champ[champ <= min(zlev)] = min(zlev)
  ...
  if (legend)
    image.plot(dum[, length(latF):1], col = col11[length(col11):1], legend.only = TRUE, zlim = range(zlev))
}

## Trace une carte 'longitude-latitude' de contours d'un champ, avec la ligne de continents qui correspond
"image.cont.c" <- function(lonF, latF, champ, titre = "", Ichange = numeric(0), zlev = pretty(champ, 10), transpose = TRUE,
                           mar = c(5, 5, 5, 6), xlab = "Longitude", ylab = "Latitude", col = "blue", add = FALSE, paquet = "fields", lty = 1) {
...
### Trace une carte 'longitude-latitude' de contours d'un champ d'anomalies, (donc centre en 0) avec la ligne de
### continents qui correspond Les isolignes <0 sont en pointilles et le 0 en gras P. Yiou, Oct. 2019.
"image.cont.c.ano" <- function(lonF, latF, champ, titre = "", Ichange = numeric(0), zlev = pretty(champ, 10), transpose = TRUE,
                               mar = c(5, 5, 5, 6), xlab = "Longitude", ylab = "Latitude", colo = "blue", add = FALSE, paquet = "fields", lty = 1) {
  # zlev=seq(-0.85,0.6,length=11)
  par(mar = mar)
  if (length(Ichange) > 0)
    champ[Ichange] = 1
  ...
  llwd = rep(1, nlev)
  llwd[zlev == 0] = 2
  contour(lonF, sort(latF), dum[, latF.sort$ix], xlab = xlab, ylab = ylab, main = titre, col = colo, add = add, nlevels = nlev,
...
"trace.po" <- function(titre = "", xcontrib, lon = lon, lat = lat, legende = TRUE, qq = NULL, col = NULL, lonlim = c(-10,
                                                                                                                     30), latlim = c(35, 60), mar = c(3, 3, 3, 8), cexpo = 3) {
  # layout(matrix(1:2,1,2),widths=c(4,1))
  library(maps)
  require(fields)
  xmoy = xcontrib
  if (is.null(col))
    col6 = rainbow(6, start = 4/6, end = 0) else col6 = col
  if (is.null(qq))
    qq = quantile(xmoy, seq(0, 1, length = length(col6)), na.rm = TRUE)
  l.col = c()
  for (i in 1:length(xmoy)) l.col = c(l.col, which(xmoy[i] >= qq[1:(length(qq) - 1)] & xmoy[i] < qq[2:length(qq)]))
  par(mar = mar)
  plot(lonlim, latlim, type = "n", xlab = "", ylab = "", main = titre)
  # world(add=TRUE)
  map(add = TRUE)
  points(lon, lat, cex = cexpo, col = "black", bg = col6[l.col], pch = 21)
  if (legende) {
...
# Fonction generale pour tracer une carte avec des points de couleur sur des valeurs discretes
trace.po2 <- function(titre = "", Xm, lon = lon, lat = lat, ncolor = 5, col = NULL) {
  library(maps)
  xmoy = Xm
  if (is.null(col))
    col6 = rainbow(ncolor, start = 4/6, end = 0) else col6 = col
  plot(seq(-10, 30, length = 10), seq(35, 60, length = 10), type = "n", xlab = "Longitude", ylab = "Latitude", main = titre)
  # world(add=TRUE)
  map(add = TRUE)
...
"image.cont.disc" <- function(lonF, latF, champ, titre = "", col = c(1:4), pref = "", leg = NULL) {
  opar = par()
  ...
  if (is.null(leg))
    leg = paste(pref, c(1:length(col)))
  legend(0, 0.5, leg, col4)
  layout(matrix(1, 1, 1))
}

# Trace de plusieurs cartes (mchamp)
"image.mfield" <- function(lonF, latF, mchamp, mchamp2 = NULL, titre = "", nplot = 4, rng = range(mchamp[, 1:nplot])) {
  ...
  for (i in 1:nreg) {
    ...
    image(lonF, latF, t(matrix(mchamp[, i], length(latF), length(lonF))), breaks = breaks, xlab = "Longitude", ylab = "Latitude",
          main = paste(titre, i), col = col10)
    if (!is.null(mchamp2))
      contour(lonF, latF, t(matrix(mchamp2[, i], length(latF), length(lonF))), add = TRUE)
    world(xlim = range(lonF), ylim = range(latF), add = TRUE)
  }
  layout(matrix(1, 1, 1))
}
...
"map_circle" <- function(Xdat, IF, infoKNMI) {
  plot(seq(-10, 30, length = 10), seq(35, 60, length = 10), type = "n", xlab = "Longitude", ylab = "Latitude", main = "")
  world(add = TRUE)
  for (i in 1:length(IF)) {
    ...
    if (dat.SEA.can[ireg + (i - 1) * 5 + 1] - dat.SEA.tot[ireg + (i - 1) * 5 + 1] > 0)
      col = "blue" else col = "red"
    cex = 50 * abs(dat.SEA.can[ireg + (i - 1) * 5 + 1] - dat.SEA.tot[ireg + (i - 1) * 5 + 1])
    points(lon.sta[1] + 0.01 * lon.sta[2], lat.sta[1] + 0.01 * lat.sta[2], cex = cex, col = col)
  }
