...
means$vals <- as.numeric(means$vals)
means$grp.mean<- ave(means$vals, means$group) ## mean for each group
...
#### select sim of one year ####
days_sim <- as.data.frame(do.call(rbind,simu.dyn$l.X$`1962`))     ### !!! mettre y-1 en année
days_sim[,1] <- as.Date(as.character(days_sim[,1]),format="%Y%m%d")
# days_sim <- days_sim[,c(2,1)]
days_sim$y.sim <- as.numeric(format(days_sim$t.sim,"%Y"))
days_sim$y.sim[as.numeric(format(days_sim$t.sim,"%m")) %in% c(1:2)] <- days_sim$y.sim[as.numeric(format(days_sim$t.sim,"%m")) %in% c(1:2)]-1

p <- ggplot(days_sim,aes(x=days_sim[,3]))
...
p <- ggplot(years, aes(x=vals, color=group,fill=group)) + geom_histogram(position = "identity",alpha=0.2)+
  theme_linedraw()+ theme(legend.position="top",legend.title=element_blank())+labs(x="Temperature (°C)",y="Count")
p
