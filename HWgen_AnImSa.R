...
## Pascal Yiou (LSCE), June 2016, Feb 2018, June 2019, November 2019
## Camille Cadiou (LSCE), October 2021
...
## Poids relatif au jour calendaire pour respecter le cycle saisonnier  
      ...
      diff.ana.cal.1 = abs(ana.d1.cal - ((t0.cal+i)%%366+1))
      # if ((diff.ana.cal%/%183)==1) {diff.ana.cal = 365-diff.ana.cal; } #permet de calculer la différnece à cheval sur deux années
      diff.ana.cal.2 = 365-diff.ana.cal.1
      diff.ana.cal = min(diff.ana.cal.1,diff.ana.cal.2) 
      weight.cal = exp(-alpha.cal*diff.ana.cal/lanamax)
      sdum=sum(weight.cal,na.rm=TRUE)
      weight.cal = weight.cal/sdum
      cat(paste("d1: ",ana.d1.cal,"\n",sep=""))
      cat(paste("da: ",(t0.cal+i)%%366,"\n",sep=""))
      cat(paste("diff: ",diff.ana.cal,"\t",diff.ana.cal.1,"\t",diff.ana.cal.2,"\t",((diff.ana.cal.1%/%183)==1),"\n",sep=""))
      cat("\n")

## Poids sur la valeur de la temperature (pour avoir l'analogue le plus chaud)  
      d1=ana.d1
      Idum = match(ana.d1,TN$Date)
      TN.d1 = TN[Idum,2] ## Temperatures analogues
...
save(file=fname,simu.dyn,alpha.cal,alpha.TN,args,weight.T)
